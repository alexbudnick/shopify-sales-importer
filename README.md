...
## v1.10.1 ShipStation Order Location Run

Adds live-safe endpoint:

- `/jobs/shipstation-order-location-repair/run?secret=...`

Safety behavior:

- Respects `DRY_RUN=true`; if DRY_RUN is true, the run endpoint previews without writing.
- Only scans configured open statuses.
- Updates only order items with a safe SKU and Airtable Location.
- Writes SKU and warehouseLocation directly to ShipStation order items.
- Uses ShipStation `/orders/createorder` with the existing orderKey to update the open order.


## v1.10.2 ShipStation Full Order Update

Improves the live ShipStation location repair endpoint.

Change:

- Before posting an update, the app now fetches the full order using `/orders/{orderId}`.
- It modifies only the matching order item's `sku` and `warehouseLocation`.
- It then sends the full order resource back through `/orders/createorder`.

Reason:

- ShipStation Create/Update Order does not support partial updates.
- The order list payload may not include enough data for a valid update request.
