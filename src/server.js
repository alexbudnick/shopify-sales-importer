...
function buildSalesFields({ order, lineItem, inventory, ownerRecordId, payoutPeriodRecordId }) {
  ...
  const fields = {
    ...
    "Labor Charge": normalizeCurrency(inv["Labor Charge"]),
    "Total Tech Fee": normalizeCurrency(inv["Total Tech Fee"]),
    ...
    "Estimated Platform Fee": 0,
    "Estimated Payment Processing Fee": financials.orderSubtotal * 0.03,
...
app.get("/", (req, res) => {
  res.json({
    ...
    version: "1.2.0-tech-costs-estimated-fees"
  });
});
